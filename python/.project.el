`(:project-tags ("curriculum_python")
		...
		:agenda-commands (("le" "Manager: What kind of effort and frequency does this item require? (Basics)"
				   tags "TODO<>\"past\"&EFFORT_TYPE=\"\"|TODO<>\"past\"&FREQUENCY=\"\""
				   ((org-agenda-hide-tags-regexp nil) ;; eventually hide non-project tags
				    (org-agenda-overriding-header "What kind of effort and frequency does this item require?")
				    (org-agenda-sorting-strategy '((tags category-up alpha-up)))
				    (org-agenda-view-columns-initially t)
				    (org-overriding-columns-format "%7CATEGORY(Cat) %30ITEM(Item) %7EFFORT_TYPE(Effort type) %7FREQUENCY(Frequency)")
				    (org-super-agenda-groups '((:auto-tags t)))))

				  ("ln" "Stakeholders: Must this item be finished by a certain date, or else it will fail? (Basics)"
				   tags "EFFORT_TYPE<>\"organization\"&HAS_DUE_DATE=\"\""
  ...
  ;;; Prioritization
				  ("as" "Stakeholders: How beneficial would finishing this item be compared to other items in this project? (Prioritization)" tags  "EFFORT_TYPE<>\"organization\""
				  ...
				  ("ae" "Team: How much effort would this item take compared to other items in this project? (Prioritization)" tags "EFFORT_TYPE<>\"organization\"&PRIORITY<>\"C\"|HAS_DUE_DATE=\"yes\""
				  ...
				  ("an" "Manager: How much impact would this item have on the budget compared to other items in this project? (Prioritization)" tags  "EFFORT_TYPE<>\"organization\"&PRIORITY=\"A\"|EFFORT_TYPE<>\"organization\"&PRIORITY=\"B\"&EFFORT_AMOUNT=\"less\"|HAS_DUE_DATE=\"yes\""
				  ...
				  ("ac" "All: Are we going to commit effort and/or budget to this? (Prioritization)" tags  "EFFORT_TYPE<>\"organization\"&PRIORITY=\"A\"|EFFORT_TYPE<>\"organization\"&PRIORITY=\"B\"&EFFORT_AMOUNT=\"less\"|HAS_DUE_DATE=\"yes\""
				   ((org-agenda-cmp-user-defined 'my-org-cmp-effort-amount)
		    ...
		    ;;; Activity
				  ("ts" "Team: What's on our calendar? (Activity)" agenda
				   ...
				   ((org-agenda-overriding-header "What's on the calendar?")
				    ...
				    (org-agenda-view-columns-initially t)
				    (org-overriding-columns-format "%7TODO(Timing) %70ITEM(Item) %15TAGS(Project)")))
				  
				  ("te" "Team: What should we do now? (Activity)" ;; (agenda
				   ;;  "HAS_DUE_DATE=\"yes\""
				   ;;  ((org-agenda-overriding-header "What's coming up in the near future?")
				   ;;   (org-agenda-sorting-strategy '((agenda timestamp-up)))
				   ;;   (org-agenda-span 'week)
				   ;;   (org-agenda-use-time-grid nil)
				   ;;   (org-agenda-view-columns-initially nil)
				   ;;   (org-overriding-columns-format "%5CATEGORY(Cat) %7TODO(Timing) %40ITEM(Item) %8EFFORT_AMOUNT(Effort) %15TAGS(Project)") 
				   ;;   ))
				  ...
				  ("tc" "Team: What references need to be processed? (Activity)"
				   tags "EFFORT_TYPE=\"organization\""
				   ((org-agenda-overriding-header "What references need to be processed?")
				    (org-agenda-sorting-strategy '((tags alpha-up)))
				    (org-agenda-view-columns-initially t)
				    (org-overriding-columns-format "%100ITEM(Item)")
				    (org-super-agenda-groups '((:auto-tags t)))))))
